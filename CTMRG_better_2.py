...
def __rot3env(env):
    ...
    buff = env['C_NW_A']
    env['C_NW_A'] = env['C_NE_A']
    env['C_NE_A'] = env['C_SE_A']
    env['C_SE_A'] = env['C_SW_A']
    env['C_SW_A'] = buff

    buff = env['C_NW_B']
    env['C_NW_B'] = env['C_NE_B']
    env['C_NE_B'] = env['C_SE_B']
    env['C_SE_B'] = env['C_SW_B']
    env['C_SW_B'] = buff
...
def CTMRGstep(A, B, chi, maxiter=1000, env={}, invprecision=1e-10, precision=1e-20, ifprint=False, ifrandom=False):
    ...
    for iter in range(maxiter):
        ...
        for itern in range(4):
            print("iter: ", iter,"\t",itern)
            env = __CTMRT_left(A, B, chi, env, invprecision, ifprint, ifrandom)
            env = __rot3env(env)
            A = __rot1(A)
            B = __rot1(B)
        ...
        print("Errorz =",errora,"\t",errorb)
        if (errora<precision and errorb<precision) or ((np.abs(errorb-preverrorb)/errorb < 1e-4 or np.abs(errora-preverrora)/errora < 1e-4) and False):
...
def __CTMRT_left(A, B, chi, env={}, invprecision=1e-10, ifprint=False, ifrandom=False):
    ...
    #     chi * D * D, chi)).T
    C_NW_A_new = CalcCorner1(P1A,C_NW_B,E_N_A)
    C_NW_B_new = CalcCorner1(P1B,C_NW_A,E_N_B)
    ...
    #     chi * D * D, chi)
    C_SW_A_new = CalcCorner2(P2B,C_SW_B,E_S_A)
    C_SW_B_new = CalcCorner2(P2A,C_SW_A,E_S_B)
    ...
    E_W_A_new = CalcEdge(P2A,E_W_B,A,A.conj(),P1B)
    E_W_B_new = CalcEdge(P2B,E_W_A,B,B.conj(),P1A)
