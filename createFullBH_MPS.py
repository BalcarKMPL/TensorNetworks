...
if __name__ == '__main__':
    ...
    dir = "./BH_short_MPS_" + str(dt) + str("_") + str(J) + str("_") + str(U)
    print("Saving in ",dir)
    ...
    for i in range(0, n + 1):
        ...
        if i > 0:
            ...
            np.savez(dir + ('/PEPS_{:05d}.npz'.format(i)), A=PEPS['A'], B=PEPS['B'], NTUerror=PEPS['NTUerror'], iter=i, dt=dt)
        ...
        np.savez(dir + ('/RHOA_{:05d}.npz'.format(i)), rhoA=env['rhoA'], rhoB=env['rhoB'], E_E_A=env['E_E_A'],
                 ...
                 C_NE_A=env['C_NE_A'], C_SE_B=env['C_SE_B'], error=env['error'], iter=i, dt=dt)
