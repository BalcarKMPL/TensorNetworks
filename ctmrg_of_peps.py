...
if __name__ == '__main__':
    env = {}
    dirs = ['./BH_NEWNTUd=5_sud_28_7_1.0_4.9_0.001_500']
    for dir in dirs:
        # peps60 --- chi=23
        indexes = [140]

        chi = int(dir.split('_')[3])*0+36
        print(indexes)
        ...
        for pepspath in paths:
            ...
            if True or not os.path.exists(dir + "/RHOA_" + pepspath.name[5:10] + ".npz"):
                ...
                for i in range(15):
                    print("CTMRG-ing PEPS nr:", int(pepspath.name[5:10]))
                    # env0 = env
                    env = CTMRG_better.CTMRGstepL(PEPS['A'], PEPS['B'], chi, env0=env0, maxiter=10, invprecision=1e-10,
