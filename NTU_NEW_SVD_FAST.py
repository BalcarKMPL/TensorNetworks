...
def __step(PEPS, precision=1e-15, ifsvdu=False, maxiter=200, ifprint=True, precisionspeed=0):
    ...
    def calc_MA_grand(gA, JA):
        ...
        j = JA.reshape(DDr)
        P = ((j.conj() @ U) * (Vh @ j)).real
        print(P/P[0])
        print(s/s[0])
        optind = np.argmax(np.where(P < 0, 1, 0))
        optind = optind if optind!=0 else DDr-1
        print("Expected optimal rcond =",s[optind]/s[0])
        for rc in 1 / 10 ** np.arange(7, -np.log10(s[-1] / s[0]) + 1, 1):
            sinv = np.where(s/s[0] < rc, 0, 1/s)
            MA_temp = (Vh.conj().T @ np.diag(sinv) @ U.conj().T @ JA.reshape(DDr)).reshape(Dr, D)
    ...
    def calc_MB_grand(gB, JB):
        ...
        for rc in 1 / 10 ** np.arange(7, -np.log10(s[-1] / s[0]) + 1, 1):
            ...
            if np.abs(error_temp) < np.abs(MB_error_best):
                MB_best = MB_temp
                MB_error_best = error_temp  # else:  #     break
