...
if __name__ == '__main__':
    ...
    d = 3
    D = 5  # tak o rzucone
    r = 5  # <14 <- wartości singularne do dt^3
    ...
    dir = "./LOCAL_LR_BH_PEPS_" + str(dt) + str("_") + str(J) + str("_") + str(U)
    print("Saving in ",dir)
    print("Init params created")
    PEPS = BoseHubbard.TrotterGate(d, r, dt/8, J=J, U=U, mu=0)
    ...
    for i in range(n + 1):
        ...
        np.savez(dir + ('/PEPS_{:05d}.npz'.format(i + 1)), A=PEPS['A'], B=PEPS['B'])

    PEPS = {}
    for i in range(-1, n + 1):
        print("#####", i + 1, "/", n + 1, " CTMRG:")
        PEPS = np.load(dir + ('/PEPS_{:05d}.npz'.format(i + 1)))
    ...
    # for i in range(-1, n + 1):
    #     print("#####", i + 1, "/", n + 1, " Exact Corelations:")
    #     PEPS = np.load(dir + ('/PEPS_{:05d}.npz'.format(i + 1)))
    #     env = np.load(dir + ('/RHOA_{:05d}.npz').format(i + 1))
    #
    #     # Korelacje dist = 0
    #
