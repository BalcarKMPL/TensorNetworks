...
def H(d, J=0, U=1, mu=0):
    ...
    HU = U / 2 * (np.einsum('ij,kl->jlik', nn - n, I) + np.einsum('ij,kl->jlik', I, nn - n)) / 2 - mu * (
            np.einsum('ij,kl->jlik', n, I) + np.einsum('ij,kl->jlik', I, n)) / 2
...
def TrotterGate(d, r, dt, J=0, U=1, mu=0):
    ...
    GA = ncon([GU, GA, GU], ([2, -1], [2, 1, -3], [-2, 1]))
    GB = ncon([GU, GB, GU], ([2, -1], [2, 1, -3], [-2, 1]))
