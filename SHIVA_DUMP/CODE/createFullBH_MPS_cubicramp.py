...
if __name__ == '__main__':
    ...
    dir = "./BH_short_MPS_CUBICRAMP_" + str(dt) + str("_") + str(tQ) + str("_") + str(n)
    print("Saving in ", dir)
    ...
    def J_t(t):
        x = t / tQ
        if t < 0: return J_crit * (1 + x - 4 / 27 * x**3)
        else: return J_crit * (1 + x)
    ...
    for i in range(0, n + 1):
        ...
        np.savez(dir + ('/RHOA_{:05d}.npz'.format(i)), rhoA=env['rhoA'], rhoB=env['rhoB'], E_E_A=env['E_E_A'],
                 ...
                 C_NE_A=env['C_NE_A'], C_SE_B=env['C_SE_B'], error=env['error'], A=PEPS['A'], B=PEPS['B'],
                 NTUerror=PEPS['NTUerror'], iter=i,
