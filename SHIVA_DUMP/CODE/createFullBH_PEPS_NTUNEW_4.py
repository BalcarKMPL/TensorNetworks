...
if __name__ == '__main__':
    ...
    dir = "./BH-NTU4-CTM4-nowenowe_" + str(chi) + str("_") + str(D) + str("_") + str(J) + str("_") + str(U) + str("_") + str(dt) + str(
        "_") + str(n)
    print("Saving in ", dir)
    ...
    for i in range(0, n + 1):
        ...
        print("#####", i, "/", n, " CTMRG:")
        env = CTM.CTMRGstepLtest(PEPS['A'], PEPS['B'], chi, env0=env, maxiter=maxiter, invprecision=INVprecision, precision=CTMRGprecision,ifrandom=True, tests1=[{'name':'N','A':ah @ a,'B':ah @ a},{'name':'NN','A':ah @ a@ah @ a,'B':ah @ a@ah @ a}], tests2=[{'name':'AhA','A':ah,'B':a},{'name':'AAh','A':a,'B':ah},{'name':'NN','A':ah @ a,'B':ah @ a}])
