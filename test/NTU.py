...
def __step(PEPS, maxiter=1000, method="L", ifsvdu=False, ifprint=False, precision=1e-20):
    ...
    for iteration in range(maxiter):
        ...
        if method == 'L':
            ...
            MA = np.linalg.solve(gA.reshape(D * D * r, D * D * r).T, JA.reshape(D * D * r))
        MA = MA.reshape(D * r, D)

        preverror = error
        error = CalculateError()
        if ifprint: print("\t\terror = ", CalculateError())

        # # Metoda alternatywna (D,Dr)
        # if method == 'L':
        #     MA = np.linalg.lstsq(gA.swapaxes(0, 1).swapaxes(2, 3).reshape(D * D * r, D * D * r).T,
        #                          JA.swapaxes(0, 1).reshape(D * D * r), rcond=1e-6)[0]
        # else:
        #     MA = np.linalg.solve(gA.swapaxes(0, 1).swapaxes(2, 3).reshape(D * D * r, D * D * r).T,
        #                          JA.swapaxes(0, 1).reshape(D * D * r))
        # MA = MA.reshape(D, D * r).T

        MA, MB = truncate2(MA @ MB.T, D)
        MB = MB.T

        # tensors = [B, B.conj(), A, A.conj(), B, B.conj(), B, B.conj(), A, A.conj(), A, A.conj(), QA, QA.conj(), QB,
        #            QB.conj(), RA, RA, MA.conj()]
        # connects = [[15, 14, 17, 16, 2], [15, 13, 18, 16, 2], [10, 9, 12, 14, 1], [10, 9, 11, 13, 1],
        #             [21, 19, 23, 22, 3], [21, 20, 23, 22, 3], [24, 26, 28, 29, 4], [25, 27, 28, 29, 4],
        #             [37, 31, 30, 26, 5], [38, 31, 30, 27, 5], [34, 33, 32, 36, 6], [34, 33, 32, 35, 6],
        #             [17, 41, 24, 19, 7], [18, 42, 25, 20, 7], [12, 36, 37, 40, 8], [11, 35, 38, -2, 8], [41, 39],
        #             [40, 39], [42, -1]]
        # con_order = [15, 16, 2, 21, 23, 22, 3, 28, 29, 4, 42, 10, 9, 1, 31, 30, 5, 26, 27, 20, 19, 7, 34, 33, 32, 6, 14,
        #              13, 39, 17, 18, 25, 24, 40, 12, 41, 37, 36, 11, 38, 8, 35]
        ...
        # X = gB.reshape(D * D * r, D * D * r)
        # print("hermitowskość: ", norm(X - X.conj().T))

        if method == 'L':
            # MB = np.linalg.lstsq(gB.swapaxes(0, 1).swapaxes(2, 3).reshape(D * D * r, D * D * r).T,
            #                      JB.swapaxes(0, 1).reshape(D * D * r), rcond=1e-6)[0]
            # MB = MB.reshape(D * r, D)
            MB = np.linalg.lstsq(gB.reshape(D * D * r, D * D * r).T,
                                 JB.reshape(D * D * r), rcond=1e-6)[0]
            MB = MB.reshape(D, D * r).T
        else:
            MB = np.linalg.solve(gB.swapaxes(0, 1).swapaxes(2, 3).reshape(D * D * r, D * D * r).T,
                                 JB.swapaxes(0, 1).reshape(D * D * r))
            MB = MB.reshape(D * r, D)
